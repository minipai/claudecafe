...
| Tool transform | Adds `set_expression`; calls persist a GIF-backed face for the calling session, then publish it in a typed RPC event to the TUI. Its face enum is built from the installed GIF filenames when the plugin loads. |
| Café RPC | Gives a newly mounted TUI the face of a given session and streams later changes. State is per session, so every window keeps its own portrait. |
