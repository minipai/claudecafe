...
## maids/ is a build artifact

`maids/` (the bundled cast, gitignored) is generated by `build.py` — a plain
copy of `packages/maid-personas` from this monorepo, one directory per language
(`maids/en`, `maids/zh`), no node toolchain anywhere:
...
- Praising / 邀功 lives inside each persona file (`maid-personas/<lang>/*.md`) — it's
  character behaviour, not a shared util.
- Which translation goes on shift follows the reply language, because a persona is
  mostly tone by example and the examples are quoted lines: a Chinese maid told to
  answer in English has to translate her own register, and that is where a persona
  goes flat. `cast_lang` overrides the guess; an untranslated maid falls back to
  English rather than dropping out of the café.
