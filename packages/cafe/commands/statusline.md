...
in `~/.config/claudecafe/config.json` (generation is off until someone can see it;
it spends API credit — one `claude -p --model haiku` call per shot).

The renderer is `~/.config/claudecafe/bin/statusbar.py` (a symlink the plugin
maintains — always point at it, never at a versioned plugin path):

- `python3 ~/.config/claudecafe/bin/statusbar.py` — both rows
- `python3 ~/.config/claudecafe/bin/statusbar.py 1` — the scene only
- `python3 ~/.config/claudecafe/bin/statusbar.py 2` — the dialogue only
...
After wiring the display, set `"look": true` in `~/.config/claudecafe/config.json`
